...
- **客户在任何位置都**不会假定应用将仅用于开发它的城市或国家/地区！ 适用于 GPS 坐标、日期和时间值和货币的代码可能会受到客户位置和区域设置的影响。 所有平台都提供了一个模拟器，可让你指定不同的位置和区域设置-使用它来测试其他半球中的位置，以及设置日期和货币格式的区域性。 纬度和经度值可以是正数或负数，小数分隔符可以是句点或逗号，并且可以通过多种方式设置日期的格式-处理！
